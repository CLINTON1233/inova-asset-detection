...
# ==================== GET COMPLETED PREPARATIONS (SESSIONS) ====================
@assets_bp.route('/api/assets/completed-sessions', methods=['GET'])
def get_completed_sessions():
    """Mendapatkan daftar session yang sudah memiliki asset (sudah divalidasi)"""
    conn = None
    try:
        conn = get_conn()
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        
        # Ambil semua devices preparations yang memiliki asset (sudah divalidasi)
        cur.execute("""
            SELECT 
                dsp.id_preparation,
                dsp.checking_number,
        ...
        # Ambil semua materials preparations yang memiliki asset (sudah divalidasi)
        cur.execute("""
            SELECT 
                msp.id_preparation,
                msp.checking_number,
        ...
        for prep in devices_preparations:
            ...
            all_preparations.append(prep_dict)
        ...
        print(f"Found {len(all_preparations)} sessions with assets")
...
@assets_bp.route('/api/assets/sessions-with-assets', methods=['GET'])
def get_sessions_with_assets():
    """Mendapatkan daftar session yang sudah memiliki asset (sudah divalidasi)"""
    conn = None
    try:
        conn = get_conn()
        ...
        # Ambil semua devices preparations yang memiliki asset
        cur.execute("""
            SELECT 
                dsp.id_preparation,
                dsp.checking_number,
        ...
        # Ambil semua materials preparations yang memiliki asset
        cur.execute("""
            SELECT 
                msp.id_preparation,
                msp.checking_number,
        ...
        print(f"Error getting sessions with assets: {e}")
        print(traceback.format_exc())
        return jsonify({
            'success': False,
            'error': str(e)
        ...
        if prep_type == 'device':
            cur.execute("""
                SELECT 
                    ...
                    dip.item_number,
                    dip.serial_number as item_serial,
                    si.device_name,
                    si.brand as device_brand,
                    si.vendor as device_vendor,
                    si.model as device_model,
                    si.specifications as device_specs
                FROM assets a
                ...
                LEFT JOIN users u ON a.validated_by = u.id_user
            ...
            cur.execute("""
                SELECT 
                    ...
                    u.username as validated_by_name,
                    mip.item_number,
                    mip.scan_code as item_scan_code,
                    si.material_name,
                    si.vendor as material_vendor,
                    si.uom as material_uom,
                    si.material_detail as material_specs
                FROM assets a
                LEFT JOIN validations v ON a.validation_id = v.id_validation
...
# @assets_bp.route('/api/assets', methods=['GET'])
# def get_all_assets():
#     """Mendapatkan daftar semua assets dengan brand dan vendor"""
#     conn = None
#     try:
#         conn = get_conn()
#         cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
...
#                 -- Ambil brand/vendor dari scanning items
#                 CASE 
#                     WHEN a.category = 'Device' THEN si.brand
#                     WHEN a.category = 'Material' THEN si.vendor
#                     ELSE NULL
#                 END as brand,
#                 CASE 
#                     WHEN a.category = 'Device' THEN si.model
#                     ELSE NULL
#                 END as model,
#                 CASE 
#                     WHEN a.category = 'Device' THEN si.vendor
#                     ELSE si.vendor
#                 END as vendor
#             FROM assets a
#             LEFT JOIN validations v ON a.validation_id = v.id_validation
...
#             LEFT JOIN materials_items_preparation mip ON v.material_item_preparation_id = mip.id_item_preparation
#             LEFT JOIN devices_scanning_items si ON dip.scanning_item_id = si.id_item
#             LEFT JOIN materials_scanning_items msi ON mip.scanning_item_id = msi.id_item
